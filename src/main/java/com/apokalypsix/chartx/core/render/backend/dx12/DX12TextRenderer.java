...
/**
 * Batched text renderer using texture atlas for DirectX 12.
 *
 ...
 */
public class DX12TextRenderer implements TextRenderer {

    private static final Logger log = LoggerFactory.getLogger(DX12TextRenderer.class);
    ...
    private DX12Shader textShader;
    private boolean resourcesInitialized = false;

    private final Map<Integer, DX12FontAtlas> atlasCache = new HashMap<>();
    private DX12FontAtlas currentAtlas;
    ...
    public DX12TextRenderer(DX12RenderDevice device, DX12ResourceManager resources) {
        this.device = device;
        this.resources = resources;
        this.font = new Font(fontFamily, Font.PLAIN, (int) fontSize);
    }

    public DX12TextRenderer(DX12RenderDevice device, DX12ResourceManager resources, float fontSize) {
        this.device = device;
        this.resources = resources;
    ...
    private void initializeResources() {
        ...
        textShader = (DX12Shader) resources.getShader(ResourceManager.SHADER_TEXT);
        ...
        vertexBuffer = (DX12Buffer) resources.getOrCreateBuffer("dx12_text_renderer", bufferDesc);

        batchData = new float[INITIAL_CAPACITY * VERTICES_PER_QUAD * FLOATS_PER_VERTEX];

        resourcesInitialized = true;
        log.debug("DX12TextRenderer resources initialized");
    }

    private DX12FontAtlas getOrCreateAtlas() {
        int key = (int) fontSize;
        DX12FontAtlas atlas = atlasCache.get(key);
        if (atlas == null) {
            atlas = new DX12FontAtlas(font);
    ...
    private void buildTextQuads(String text, float x, float y, Color color) {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            DX12FontAtlas.GlyphInfo glyph = currentAtlas.getGlyph(c);
    ...
    private void renderBatch() {
        DX12Texture atlasTexture = currentAtlas.getTexture();
    ...
    private static float[] createOrthoMatrix(float left, float right, float bottom, float top) {
    ...
    @Override
    public void dispose() {
        for (DX12FontAtlas atlas : atlasCache.values()) {
            atlas.dispose();
        }
        atlasCache.clear();
